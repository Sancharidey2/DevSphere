...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const nextButton = document.getElementById("next-btn") as HTMLElement;
  const gameContainer = document.querySelector(".game-container") as HTMLElement;
  
  const audioElement = new Audio('sounds/1.mp3');
  ...
  nextButton.addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      gameContainer.className = `game-container ${scenes[currentSceneIndex].backgroundClass}`;
