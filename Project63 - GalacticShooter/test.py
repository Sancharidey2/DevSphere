...
def listen_for_pause():
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            if 'pause' in command:
                return True  # Return True if 'pause' is detected
        except sr.UnknownValueError:
            # If speech is not recognized, we continue the loop
            pass
...
# Function to check for voice command and control the game state
def check_voice_control():
    global game_paused
    while True:
        if listen_for_pause():
            if game_paused:
        ...
        time.sleep(0.5)  # Add a small delay to avoid constant checking

voice_thread = threading.Thread(target=check_voice_control)
voice_thread.daemon = True  # Run this thread as a daemon
voice_thread.start()
...
def game_loop():
    ...
    while running:
        ...
        pygame.display.flip()
        clock.tick(45)  # 60 frames per second

        game_paused = False

        while True:
            if game_paused:
                # Show paused screen or stop game actions
                display_paused_screen()
            else:
                # Run game updates, player movements, physics, etc.
                update_game_state()
                move_player()
                render_game()
